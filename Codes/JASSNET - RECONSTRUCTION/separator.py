...
class ConvolutionModule(nn.Module):
    """
    Fine-grained local feature block coerente con Fig. 3 di JASSNet:
    ...
    La skip connection bypassa la seconda depthwise convolution,
    dopo la proiezione lineare e la ReLU.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.dw_in(out.transpose(1, 2)).transpose(1, 2)

        out = self.activation(self.linear(out))

        residual = out

        out = self.dw_out(out.transpose(1, 2)).transpose(1, 2)
...
class RotaryPositionEmbedding(nn.Module):
    """
    Rotary Positional Embedding applicato a tensori [B, S, D].
    La dimensione D deve essere pari.
    """
...
class JointLocalGlobalAttention(nn.Module):
    """
    Joint local-global attention coerente con le Eq. 6-8 di JASSNet.

    La rappresentazione condivisa:
        Z = ConvM(X'') in R^(S x D), con D << N

    viene utilizzata per costruire:
        Q, K   -> attenzione locale
        Q', K' -> attenzione globale linearizzata
    """
    ...
    def __init__(
        ...
        # Terzo convolution module della Fig. 4:
        # X'' -> Z, con D << N
...
class CardiopulmonarySeparator(nn.Module):
    ...
    def __init__(
        ...
        self.num_modules = cfg.JASSNET_NUM_MODULES
        ...
        self.modules_stack = nn.ModuleList(
            [
                SeparationModule(
                    ...
                    attn_dim=cfg.JASSNET_ATTN_DIM,
                    local_chunk=cfg.JASSNET_LOCAL_CHUNK,
                    dropout_p=dropout_p,
                )
