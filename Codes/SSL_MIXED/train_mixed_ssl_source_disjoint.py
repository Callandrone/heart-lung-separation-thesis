...
SSL_TRAIN_SCRIPT_VERSION = "SSL_PSEUDO_THREE_DOMAIN_2026_06_24_V1"
...
try:
    from domain_adaptive_replay_augmentation import TorabiMorphReplayDataset
except Exception:
    TorabiMorphReplayDataset = None
...
def ssl_pseudo_loss(
    ...
    L_ref: torch.Tensor,
    M: torch.Tensor,
    conf_w: torch.Tensor | None = None,
) -> tuple[torch.Tensor, dict]:
    """
    Loss SSL per V1 pseudo-label.

    Usa:
    - pseudo supervised loss verso H_pseudo/L_pseudo;
    - confidence weighting sample-wise;
    - mixture consistency input-only.

    Non usa ground truth V1.
    """
    eps = 1e-8

    if conf_w is None:
        conf_w = H_hat.new_ones((H_hat.shape[0],))
    else:
        conf_w = conf_w.view(-1).float()

    # SI-SDR per sample
    loss_sisdr_h = -si_sdr(H_hat, H_ref)
    loss_sisdr_l = -si_sdr(L_hat, L_ref)
    loss_sisdr = 0.5 * (loss_sisdr_h + loss_sisdr_l)

    # RMS per sample
    loss_rms_h = rms_loss_per_sample(H_hat, H_ref)
    loss_rms_l = rms_loss_per_sample(L_hat, L_ref)
    loss_rms = 0.5 * (loss_rms_h + loss_rms_l)

    # polarity per sample
    loss_pol_h = polarity_loss_per_sample(H_hat, H_ref)
    loss_pol_l = polarity_loss_per_sample(L_hat, L_ref)
    loss_pol = 0.5 * (loss_pol_h + loss_pol_l)

    lambda_rms = float(getattr(cfg, "LAMBDA_RMS", 1.0))
    lambda_pol = float(getattr(cfg, "LAMBDA_POLARITY", 0.1))
    ...
    pseudo_loss = (conf_w * pseudo_per_sample).sum() / (conf_w.sum() + eps)

    lambda_ssl_mix = float(getattr(cfg, "SSL_LAMBDA_MIX", 0.0))
    mix_loss = mixture_consistency_loss(
        mixture=M,
        H_hat=H_hat,
        L_hat=L_hat,
    )

    total = pseudo_loss + lambda_ssl_mix * mix_loss

    metrics = {
        "loss_total": float(total.item()),
        "loss_sisdr": float(loss_sisdr.mean().item()),
        "loss_sisdr_h": float(loss_sisdr_h.mean().item()),
        "loss_sisdr_l": float(loss_sisdr_l.mean().item()),
        "loss_rms": float(loss_rms.mean().item()),
        "loss_polarity": float(loss_pol.mean().item()),
        "loss_mix": float(mix_loss.item()),
        "weighted_loss_mix": float((lambda_ssl_mix * mix_loss).item()),
        "ssl_conf_weight_mean": float(conf_w.mean().item()),
        "ssl_conf_weight_min": float(conf_w.min().item()),
        "ssl_conf_weight_max": float(conf_w.max().item()),
        "lambda_ssl_mix": float(lambda_ssl_mix),
    }
...
class SSLPseudoTripletDataset(TripletDataset):
    ...
    def __init__(
        ...
        super().__init__(root_dir, sr=sr, segment_samples=segment_samples)

        self.weights_by_name = {}
    ...
    def _linear_score(self, value, bad, good, higher_is_better=True):
        ...
        if v <= good:
            return 1.0
        return (bad - v) / (bad - good + 1e-8)

    def _compute_conf_weight(self, row) -> float:
        min_w = float(getattr(cfg, "SSL_CONF_MIN_WEIGHT", 0.25))
        max_w = float(getattr(cfg, "SSL_CONF_MAX_WEIGHT", 1.0))

        corr_good = float(getattr(cfg, "SSL_CONF_CORR_GOOD", 0.985))
        corr_bad = float(getattr(cfg, "SSL_CONF_CORR_BAD", 0.950))

        nmse_good = float(getattr(cfg, "SSL_CONF_NMSE_GOOD_DB", -18.0))
        nmse_bad = float(getattr(cfg, "SSL_CONF_NMSE_BAD_DB", -10.0))

        hl_corr_bad = float(getattr(cfg, "SSL_CONF_HL_CORR_BAD", 0.85))
        abs_snr_bad = float(getattr(cfg, "SSL_CONF_ABS_SNR_BAD_DB", 12.0))

        s_corr = self._linear_score(
            row.get("final_mix_corr", np.nan),
            bad=corr_bad,
            good=corr_good,
            higher_is_better=True,
        )

        s_nmse = self._linear_score(
            row.get("final_mix_nmse_db", np.nan),
            bad=nmse_bad,
            good=nmse_good,
        ...
        try:
            abs_hl_corr = abs(float(row.get("abs_pseudo_h_l_corr", 0.0)))
        except Exception:
            abs_hl_corr = 0.5

        try:
            abs_snr = abs(float(row.get("abs_pseudo_h_over_l_snr_db", 0.0)))
        except Exception:
            abs_snr = 6.0

        penalty_corr = 1.0
        if np.isfinite(abs_hl_corr) and abs_hl_corr > hl_corr_bad:
            penalty_corr = 0.5

        penalty_snr = 1.0
        if np.isfinite(abs_snr) and abs_snr > abs_snr_bad:
            penalty_snr = 0.5

        score = 0.5 * s_corr + 0.5 * s_nmse
        score = score * penalty_corr * penalty_snr

        w = min_w + (max_w - min_w) * score
        return float(np.clip(w, min_w, max_w))

    def __getitem__(self, idx: int):
        M, H, L = super().__getitem__(idx)
        name = self.names[idx]
        w = float(self.weights_by_name.get(name, 1.0))
        return M, H, L, torch.tensor(w, dtype=torch.float32)



def _target_rand_db_to_amp(db: float) -> float:
    return float(10.0 ** (float(db) / 20.0))


def _target_rand_uniform(value) -> float:
    """Accept a scalar or a (min, max) tuple/list."""
    if isinstance(value, (tuple, list)):
        if len(value) != 2:
            raise ValueError(f"Expected scalar or length-2 range, got {value!r}")
        return random.uniform(float(value[0]), float(value[1]))
    return float(value)


def _target_rand_remove_dc(x: torch.Tensor) -> torch.Tensor:
    return x - x.mean(dim=-1, keepdim=True)


def _target_rand_rms(x: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    return torch.sqrt(torch.mean(x * x, dim=-1, keepdim=True) + eps)


def _target_rand_match_rms(
    y: torch.Tensor,
    reference: torch.Tensor,
    eps: float = 1e-8,
) -> torch.Tensor:
    return y * (_target_rand_rms(reference, eps=eps) / (_target_rand_rms(y, eps=eps) + eps))


def _target_rand_smooth_band_mask(
    freqs: torch.Tensor,
    low_hz: float,
    high_hz: float,
    transition_hz: float,
) -> torch.Tensor:
    """Raised-cosine trapezoid mask used for smooth FFT-domain shaping."""
    transition_hz = max(float(transition_hz), 1.0)
    f = freqs

    left = torch.clamp((f - (float(low_hz) - transition_hz)) / transition_hz, 0.0, 1.0)
    left = 0.5 - 0.5 * torch.cos(torch.tensor(np.pi, device=f.device) * left)

    right = torch.clamp(((float(high_hz) + transition_hz) - f) / transition_hz, 0.0, 1.0)
    right = 0.5 - 0.5 * torch.cos(torch.tensor(np.pi, device=f.device) * right)

    return left * right


def _target_rand_fft_eq(
    x: torch.Tensor,
    sr: int,
    band_gains_db: list[tuple[float, float, float]],
    transition_hz: float = 80.0,
) -> torch.Tensor:
    """
    Smooth random FFT equalisation.

    x is expected as [1, T] or [T]. The returned tensor has the original shape
    and zero mean. This is intentionally used only for training-time target
    randomization, never for validation/test.
    """
    ...
    dtype = x1.dtype
    x1 = x1 - x1.mean()
    X = torch.fft.rfft(x1.float())
    freqs = torch.fft.rfftfreq(n, d=1.0 / float(sr)).to(device=device)

    gain_db = torch.zeros_like(freqs, dtype=torch.float32, device=device)
    for low_hz, high_hz, db in band_gains_db:
        if abs(float(db)) < 1e-12:
            continue
        mask = _target_rand_smooth_band_mask(freqs, low_hz, high_hz, transition_hz)
        gain_db = gain_db + float(db) * mask

    gain = torch.pow(torch.tensor(10.0, device=device), gain_db / 20.0)
    y = torch.fft.irfft(X * gain, n=n).to(dtype=dtype)
    y = y - y.mean()
    return y.reshape(original_shape)


def _target_rand_bandpass(
    x: torch.Tensor,
    sr: int,
    low_hz: float,
    high_hz: float,
    ...
    X = torch.fft.rfft(x1.float())
    freqs = torch.fft.rfftfreq(n, d=1.0 / float(sr)).to(device=device)
    mask = _target_rand_smooth_band_mask(freqs, low_hz, high_hz, transition_hz).float()
    y = torch.fft.irfft(X * mask, n=n).to(dtype=dtype)
    y = y - y.mean()
    return y.reshape(original_shape)


def _target_rand_texture(
    x: torch.Tensor,
    sr: int,
    snr_db: float,
    low_hz: float,
    ...
    """Add weak band-limited texture at a controlled SNR relative to x."""
    x0 = _target_rand_remove_dc(x)
    noise = torch.randn_like(x0)
    noise = _target_rand_bandpass(noise, sr=sr, low_hz=low_hz, high_hz=high_hz)
    noise = _target_rand_remove_dc(noise)

    target_noise_rms = _target_rand_rms(x0, eps=eps) / _target_rand_db_to_amp(float(snr_db))
    noise = noise * (target_noise_rms / (_target_rand_rms(noise, eps=eps) + eps))
    return _target_rand_remove_dc(x0 + noise)


class TargetRandomizationDataset(Dataset):
    """
    ...
    - H/L are perturbed first, then M is rebuilt as M = H + L;
    - optional source-RMS preservation keeps the original local H/L energy ratio.

    Goal: test robustness/domain-randomization, not claim deterministic domain matching.
    """
    ...
    def __init__(
        ...
        sr: int = 4000,
        prob: float = 0.30,
        preserve_source_rms: bool = True,
        preserve_peak: bool = True,
        peak_value: float = 0.95,
        apply_ls_prob: float = 0.90,
        ls_eq_prob: float = 0.30,
        ls_eq_db: tuple[float, float] = (-2.0, 2.0),
        ls_texture_prob: float = 0.25,
        ls_texture_snr_db: tuple[float, float] = (35.0, 45.0),
        ls_texture_low_hz: float = 100.0,
        ls_texture_high_hz: float = 1200.0,
        shared_gain_prob: float = 0.30,
        shared_gain_db: tuple[float, float] = (-1.5, 1.5),
        eps: float = 1e-8,
    ):
        self.base_dataset = base_dataset
        self.sr = int(sr)
        self.prob = float(prob)
        self.preserve_source_rms = bool(preserve_source_rms)
        self.preserve_peak = bool(preserve_peak)
        self.peak_value = float(peak_value)
        self.apply_ls_prob = float(apply_ls_prob)
        self.ls_eq_prob = float(ls_eq_prob)
        self.ls_eq_db = tuple(map(float, ls_eq_db))
        self.ls_texture_prob = float(ls_texture_prob)
        self.ls_texture_snr_db = tuple(map(float, ls_texture_snr_db))
        self.ls_texture_low_hz = float(ls_texture_low_hz)
        self.ls_texture_high_hz = float(ls_texture_high_hz)
        self.shared_gain_prob = float(shared_gain_prob)
        self.shared_gain_db = tuple(map(float, shared_gain_db))
        self.eps = float(eps)
    ...
    def _augment_lung(self, L: torch.Tensor) -> torch.Tensor:
        y = _target_rand_remove_dc(L)

        if random.random() <= self.ls_eq_prob:
            # Independent mild random gains. This avoids forcing one handcrafted
            # spectrum and instead exposes the model to a local neighborhood of
            # Torabi-like LS morphologies.
            eq_db = float(_target_rand_uniform(self.ls_eq_db))
            y = _target_rand_fft_eq(
                y,
                sr=self.sr,
                band_gains_db=[
                    (50.0, 150.0, 0.50 * eq_db),
                    (150.0, 500.0, eq_db),
                    (500.0, 1000.0, eq_db),
                    (1000.0, 1800.0, 0.50 * eq_db),
                ],
                transition_hz=80.0,
            )

        if random.random() <= self.ls_texture_prob:
            snr_db = float(_target_rand_uniform(self.ls_texture_snr_db))
            y = _target_rand_texture(
                y,
                sr=self.sr,
                snr_db=snr_db,
                low_hz=self.ls_texture_low_hz,
                high_hz=self.ls_texture_high_hz,
                eps=self.eps,
            )

        return _target_rand_remove_dc(y)
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        H_aug = _target_rand_remove_dc(H)
        L_aug = _target_rand_remove_dc(L)

        if random.random() <= self.apply_ls_prob:
            L_aug = self._augment_lung(L_aug)

        if self.preserve_source_rms:
            H_aug = _target_rand_match_rms(H_aug, H, eps=self.eps)
            L_aug = _target_rand_match_rms(L_aug, L, eps=self.eps)

        M_aug = H_aug + L_aug

        if random.random() <= self.shared_gain_prob:
            gain_db = float(_target_rand_uniform(self.shared_gain_db))
            gain = _target_rand_db_to_amp(gain_db)
            M_aug = M_aug * gain
            H_aug = H_aug * gain
            L_aug = L_aug * gain
        ...
        if self.preserve_peak:
            ...
            if float(peak.item()) > self.peak_value:
...
def train_mixed_one_epoch(
    ...
    device: torch.device,
    epoch_idx: int,
    ssl_loader: DataLoader | None = None,
    lambda_l1: float = cfg.LAMBDA_L1,
    grad_clip: float = cfg.GRAD_CLIP,
) -> dict:
    """
    Three-domain conservative SSL epoch.

    Each optimisation step uses exactly one batch from one of these domains:
    - target/Torabi supervised batch, weighted strongly;
    - EXP_H synthetic replay batch, weighted lightly for retention;
    - V1 real-mixture SSL pseudo-label batch, weighted very lightly.

    The V1 SSL pseudo-labels are not treated as ground truth. They enter only as
    weak teacher targets, using the same supervised loss but with a small domain
    multiplier. Validation remains target supervised only and never includes V1.
    """
    model.train()

    p_target = get_mixed_v2_probability(epoch_idx)
    p_target = min(max(float(p_target), 0.0), 1.0)

    use_ssl = bool(getattr(cfg, "USE_SSL_PSEUDO", False)) and ssl_loader is not None
    p_ssl_remainder = float(getattr(cfg, "SSL_PSEUDO_PROB_IN_REMAINDER", 0.20))
    p_ssl_remainder = min(max(p_ssl_remainder, 0.0), 1.0)

    w_target = float(getattr(cfg, "MIXED_WEIGHT_V2", 1.0))
    w_synth = float(getattr(cfg, "MIXED_WEIGHT_SYNTH", 0.05))
    w_ssl = float(getattr(cfg, "SSL_PSEUDO_WEIGHT", 0.02))
    ...
    ssl_iter = iter(ssl_loader) if use_ssl else None

    accum = {}
    domain_counts = {"target": 0, "synthetic": 0, "ssl_pseudo": 0}

    progress = tqdm(
        range(n_steps),
        desc="Mixed+SSL training",
        leave=False,
        dynamic_ncols=True,
        mininterval=1.0,
    )
    ...
    for _ in progress:
        ...
        if r < p_target:
            ...
            domain = "target"
            domain_weight = w_target
        else:
            if use_ssl and random.random() < p_ssl_remainder:
                batch, ssl_iter = _next_cycled_batch(ssl_loader, ssl_iter)
                domain = "ssl_pseudo"
                domain_weight = w_ssl
            else:
        ...
        if isinstance(batch, (list, tuple)) and len(batch) == 4:
            M, H_ref, L_ref, ssl_conf_w = batch
        else:
            M, H_ref, L_ref = batch
        M = M.to(device, non_blocking=True)
        H_ref = H_ref.to(device, non_blocking=True)
        ...
        if ssl_conf_w is not None:
            ssl_conf_w = ssl_conf_w.to(device, non_blocking=True)

        optimizer.zero_grad(set_to_none=True)

        H_hat, L_hat = model(M)

        if domain == "ssl_pseudo":
            loss_unweighted, metrics = ssl_pseudo_loss(
                H_hat=H_hat,
                H_ref=H_ref,
                L_hat=L_hat,
                L_ref=L_ref,
        ...
        accum[f"{domain}_loss_total_weighted"] = (
            accum.get(f"{domain}_loss_total_weighted", 0.0)
            + float(loss.item())
        )
        ...
        progress.set_postfix({
            "domain": domain,
            "pT": f"{p_target:.2f}",
            "pSSLrem": f"{p_ssl_remainder:.2f}" if use_ssl else "0.00",
            "w": f"{domain_weight:.3f}",
            "loss": f"{loss.item():.4f}",
            "H": f"{-metrics['loss_sisdr_h']:.2f}dB",
            "L": f"{-metrics['loss_sisdr_l']:.2f}dB",
        })
    ...
    result["mixed_p_v2"] = p_target
    result["mixed_weight_v2"] = w_target
    result["mixed_weight_synth"] = w_synth
    result["ssl_pseudo_enabled"] = bool(use_ssl)
    result["ssl_pseudo_prob_in_remainder"] = p_ssl_remainder if use_ssl else 0.0
    result["ssl_pseudo_weight"] = w_ssl if use_ssl else 0.0
    result["mixed_v2_batches"] = domain_counts["target"]
    result["mixed_synth_batches"] = domain_counts["synthetic"]
    result["mixed_ssl_batches"] = domain_counts["ssl_pseudo"]
    result["mixed_steps_per_epoch"] = n_steps

    for domain, count in domain_counts.items():
        key_unw = f"{domain}_loss_total_unweighted"
        key_w = f"{domain}_loss_total_weighted"
        if count > 0 and key_unw in accum:
            result[key_unw] = accum[key_unw] / count
        if count > 0 and key_w in accum:
            result[key_w] = accum[key_w] / count
...
def run_finetune(
    ...
    if mixed_training:
        ...
        synth_split_csv = str(getattr(cfg, "SYNTH_SOURCE_DISJOINT_SPLIT_CSV", "")).strip()
        if synth_split_csv == "":
            default_synth_split = os.path.join(str(synth_dir), "source_disjoint_split_smoke.csv")
            if os.path.exists(default_synth_split):
                synth_split_csv = default_synth_split

        require_synth_train_split = bool(getattr(cfg, "REQUIRE_SYNTH_TRAIN_SPLIT", False))

        if synth_split_csv != "" and os.path.exists(synth_split_csv):
            synth_split_df = pd.read_csv(synth_split_csv)
            required = {"fold_no", "base_id", "split"}
            missing = required - set(synth_split_df.columns)
            if missing:
                raise RuntimeError(
                    f"Synthetic replay split CSV is missing columns: {sorted(missing)}"
                )

            synth_replay_fold = int(getattr(cfg, "SYNTH_REPLAY_FOLD_NO", 1))
            synth_replay_split = str(getattr(cfg, "SYNTH_REPLAY_SPLIT", "train")).lower().strip()

            synth_split_df = synth_split_df.copy()
            synth_split_df["base_id"] = synth_split_df["base_id"].astype(str).map(normalise_base_triplet_id)
            synth_split_df["split"] = synth_split_df["split"].astype(str).str.lower()

            synth_allowed_bases = set(
                synth_split_df[
                    (synth_split_df["fold_no"].astype(int) == synth_replay_fold)
                    & (synth_split_df["split"] == synth_replay_split)
                ]["base_id"].astype(str).tolist()
            )

            if len(synth_allowed_bases) == 0:
                raise RuntimeError(
                    "Synthetic replay train split is empty after filtering.\n"
                    f"CSV={synth_split_csv}\n"
                    f"fold={synth_replay_fold} split={synth_replay_split}"
                )

            print()
            print("=" * 76)
            print("SYNTHETIC REPLAY TRAIN-ONLY FILTER ENABLED")
            print("=" * 76)
            print(f"Synthetic split CSV : {synth_split_csv}")
            print(f"Replay fold/split   : fold={synth_replay_fold} | split={synth_replay_split}")
            print(f"Allowed replay bases: {len(synth_allowed_bases)}")

        elif require_synth_train_split:
            raise RuntimeError(
                "REQUIRE_SYNTH_TRAIN_SPLIT=True, but no valid synthetic replay split CSV was found.\n"
                "Set SYNTH_SOURCE_DISJOINT_SPLIT_CSV in model_config.py, or place "
                "source_disjoint_split_smoke.csv inside SYNTH_SUPERVISED_DIR."
            )
        else:
            print()
            print("[WARNING] Synthetic replay is NOT restricted to the original train split.")
            print("          Set REQUIRE_SYNTH_TRAIN_SPLIT=True once the split CSV path is known.")

        if bool(getattr(cfg, "USE_SSL_PSEUDO", False)):
            ssl_dir = str(getattr(cfg, "SSL_PSEUDO_DIR", "")).strip()
            if ssl_dir == "":
                raise RuntimeError("USE_SSL_PSEUDO=True requires SSL_PSEUDO_DIR in model_config.py")
            if not os.path.exists(ssl_dir):
                raise RuntimeError(f"SSL_PSEUDO_DIR does not exist: {ssl_dir}")
            if bool(getattr(cfg, "USE_SSL_CONFIDENCE_WEIGHTS", False)):
                ssl_dataset = SSLPseudoTripletDataset(
                    ssl_dir,
                    manifest_csv=str(getattr(cfg, "SSL_CONFIDENCE_MANIFEST", "")),
                    sr=args.sr,
                    segment_samples=args.seg_samples,
                )
            else:
                ssl_dataset = TripletDataset(
                    ssl_dir,
                    sr=args.sr,
                    segment_samples=args.seg_samples,
                )
            print(f"SSL confidence weights: {getattr(cfg, 'USE_SSL_CONFIDENCE_WEIGHTS', False)}")
            print(f"SSL confidence manifest: {getattr(cfg, 'SSL_CONFIDENCE_MANIFEST', '')}")
            print(f"SSL lambda mix input-only: {getattr(cfg, 'SSL_LAMBDA_MIX', 0.0)}")
            print()
            print("=" * 76)
            print("SSL PSEUDO-LABEL TRAINING ENABLED — V1 REAL MIXTURES")
            print("=" * 76)
            print(f"SSL pseudo dir        : {ssl_dir}")
            print(f"SSL pseudo segments   : {len(ssl_dataset)}")
            print(f"SSL pseudo weight     : {getattr(cfg, 'SSL_PSEUDO_WEIGHT', 0.02)}")
            print(f"SSL prob in remainder : {getattr(cfg, 'SSL_PSEUDO_PROB_IN_REMAINDER', 0.20)}")
            print("Validation policy     : V1 is NOT used in validation")

        print()
        print("=" * 76)
        print("MIXED TRAINING ENABLED: target-domain adaptation + synthetic replay + optional SSL pseudo")
        print("=" * 76)
        print(f"Target supervised dir : {args.supervised_dir}")
        ...
        print(
            "Domain weights        : "
            f"target={getattr(cfg, 'MIXED_WEIGHT_V2', 1.0)} | "
            f"synthetic={getattr(cfg, 'MIXED_WEIGHT_SYNTH', 0.3)} | "
            f"ssl_pseudo={getattr(cfg, 'SSL_PSEUDO_WEIGHT', 0.0)}"
        )
        print(
            "Mixture losses        : "
    ...
    use_source_disjoint = bool(getattr(cfg, "USE_SOURCE_DISJOINT_SPLIT", False))
    ...
    if use_source_disjoint:
        ...
        source_split_csv = getattr(cfg, "SOURCE_DISJOINT_SPLIT_CSV", None)
        if source_split_csv is None or str(source_split_csv).strip() == "":
            raise RuntimeError(
                "USE_SOURCE_DISJOINT_SPLIT=True requires SOURCE_DISJOINT_SPLIT_CSV."
            )

        if not os.path.exists(source_split_csv):
            raise RuntimeError(f"Source-disjoint split file not found: {source_split_csv}")

        source_split_df = pd.read_csv(source_split_csv)
        required_cols = {"fold_no", "base_id", "split", "hs_source_id", "ls_source_id"}
        ...
        source_split_df["base_id"] = source_split_df["base_id"].astype(str).map(normalise_base_triplet_id)
        source_split_df["split"] = source_split_df["split"].astype(str).str.lower()

        print()
        print("=" * 76)
        print("SOURCE-DISJOINT SPLIT ENABLED FOR TARGET DATASET")
        print("=" * 76)
        ...
        for fold_no in range(1, args.n_folds + 1):
            g = source_split_df[source_split_df["fold_no"].astype(int) == int(fold_no)]
            if len(g) == 0:
            ...
            train_bases_fold = sorted(set(train_g["base_id"].astype(str)) & available_bases)
            val_bases_fold = sorted(set(val_g["base_id"].astype(str)) & available_bases)
            unused_bases_fold = sorted(set(unused_g["base_id"].astype(str)) & available_bases)

            print(
                f"Fold {fold_no}: "
                f"train bases={len(train_bases_fold)} | "
                f"val bases={len(val_bases_fold)} | "
                f"unused bases={len(unused_bases_fold)} | "
            ...
            if h_overlap or l_overlap:
                raise RuntimeError(
                    f"Source leakage detected in fold {fold_no}. "
                    f"HS overlap={sorted(h_overlap)[:5]} | LS overlap={sorted(l_overlap)[:5]}"
                )

            if len(train_bases_fold) == 0 or len(val_bases_fold) == 0:
                raise RuntimeError(
                    f"Fold {fold_no}: empty train or validation base set after "
                    "intersecting with the target dataset."
                )

            fold_iterator.append((fold_no - 1, train_bases_fold, val_bases_fold))

        print(f"Target triplets (base): {len(base_ids_all)} | Total segments: {len(dataset)}")
        print(f"Cross-validation: {args.n_folds} source-disjoint fold(s)")
    ...
    for fold_idx, train_bases, val_bases in fold_iterator:
        ...
        train_bases = {str(b) for b in train_bases}
        val_bases = {str(b) for b in val_bases}
        ...
        target_train_dataset = Subset(fold_dataset, train_seg_idx)

        if bool(getattr(cfg, "AUGMENT_TARGET_RANDOMIZATION", False)):
            target_train_dataset = TargetRandomizationDataset(
                target_train_dataset,
                sr=args.sr,
                prob=float(getattr(cfg, "TARGET_RANDOM_PROB", 0.30)),
                preserve_source_rms=bool(getattr(cfg, "TARGET_RANDOM_PRESERVE_SOURCE_RMS", True)),
                preserve_peak=bool(getattr(cfg, "TARGET_RANDOM_PRESERVE_PEAK", True)),
                peak_value=float(getattr(cfg, "TARGET_RANDOM_PEAK_VALUE", 0.95)),
                apply_ls_prob=float(getattr(cfg, "TARGET_RANDOM_APPLY_LS_PROB", 0.90)),
                ls_eq_prob=float(getattr(cfg, "TARGET_RANDOM_LS_EQ_PROB", 0.30)),
                ls_eq_db=tuple(getattr(cfg, "TARGET_RANDOM_LS_EQ_DB", (-2.0, 2.0))),
                ls_texture_prob=float(getattr(cfg, "TARGET_RANDOM_LS_TEXTURE_PROB", 0.25)),
                ls_texture_snr_db=tuple(getattr(cfg, "TARGET_RANDOM_LS_TEXTURE_SNR_DB", (35.0, 45.0))),
                ls_texture_low_hz=float(getattr(cfg, "TARGET_RANDOM_LS_TEXTURE_LOW_HZ", 100.0)),
                ls_texture_high_hz=float(getattr(cfg, "TARGET_RANDOM_LS_TEXTURE_HIGH_HZ", 1200.0)),
                shared_gain_prob=float(getattr(cfg, "TARGET_RANDOM_SHARED_GAIN_PROB", 0.30)),
                shared_gain_db=tuple(getattr(cfg, "TARGET_RANDOM_SHARED_GAIN_DB", (-1.5, 1.5))),
            )

            print()
            print("=" * 76)
            print("MILD TARGET RANDOMIZATION ENABLED ON TORABI TRAIN ONLY")
            print("=" * 76)
            print(f"prob                         : {getattr(cfg, 'TARGET_RANDOM_PROB', 0.30)}")
            print(f"preserve source RMS           : {getattr(cfg, 'TARGET_RANDOM_PRESERVE_SOURCE_RMS', True)}")
            print(f"LS apply prob                 : {getattr(cfg, 'TARGET_RANDOM_APPLY_LS_PROB', 0.90)}")
            print(f"LS EQ prob / dB               : {getattr(cfg, 'TARGET_RANDOM_LS_EQ_PROB', 0.30)} / {getattr(cfg, 'TARGET_RANDOM_LS_EQ_DB', (-2.0, 2.0))}")
            print(f"LS texture prob / SNR dB      : {getattr(cfg, 'TARGET_RANDOM_LS_TEXTURE_PROB', 0.25)} / {getattr(cfg, 'TARGET_RANDOM_LS_TEXTURE_SNR_DB', (35.0, 45.0))}")
            print(f"LS texture band Hz            : {getattr(cfg, 'TARGET_RANDOM_LS_TEXTURE_LOW_HZ', 100.0)} - {getattr(cfg, 'TARGET_RANDOM_LS_TEXTURE_HIGH_HZ', 1200.0)}")
            print(f"shared gain prob / dB         : {getattr(cfg, 'TARGET_RANDOM_SHARED_GAIN_PROB', 0.30)} / {getattr(cfg, 'TARGET_RANDOM_SHARED_GAIN_DB', (-1.5, 1.5))}")
        ...
        if mixed_training:
            ...
            if synth_allowed_bases is not None:
                before_synth = len(synth_indices)
                synth_indices = [
                    i for i, name in enumerate(synth_dataset.names)
                    if name.split("_s")[0] in synth_allowed_bases
                ]
                print(
                    f"Fold {fold_no} synthetic replay train-only filter: "
                    f"{before_synth} -> {len(synth_indices)} segments"
                )

                if len(synth_indices) == 0:
                    raise RuntimeError(
                        "Synthetic replay filtering produced zero segments. "
                        "Check SYNTH_SOURCE_DISJOINT_SPLIT_CSV and base_id format."
                    )
            ...
            synth_loader_dataset = Subset(synth_dataset, synth_indices)

            if bool(getattr(cfg, "AUGMENT_REPLAY_TORABI_MORPH", False)):
                if TorabiMorphReplayDataset is None:
                    raise RuntimeError(
                        "AUGMENT_REPLAY_TORABI_MORPH=True but "
                        "domain_adaptive_replay_augmentation.py could not be imported. "
                        "Place it in the same folder as train_mixed_source_disjoint.py."
                    )

                synth_aug_dataset = TorabiMorphReplayDataset(
                    synth_dataset,
                    sr=args.sr,
                    prob=float(getattr(cfg, "TORABI_MORPH_PROB", 0.50)),
                    preserve_peak=bool(getattr(cfg, "TORABI_MORPH_PRESERVE_PEAK", True)),
                    peak_value=float(getattr(cfg, "TORABI_MORPH_PEAK_VALUE", 0.95)),
                    preserve_source_rms=bool(getattr(cfg, "TORABI_MORPH_PRESERVE_SOURCE_RMS", True)),
                    apply_ls_prob=float(getattr(cfg, "TORABI_MORPH_APPLY_LS_PROB", 0.90)),
                    ls_band_150_1000_boost_db=tuple(getattr(cfg, "TORABI_MORPH_LS_BAND_150_1000_BOOST_DB", (1.0, 4.0))),
                    ls_band_500_1000_boost_db=tuple(getattr(cfg, "TORABI_MORPH_LS_BAND_500_1000_BOOST_DB", (1.0, 5.0))),
                    ls_transient_prob=float(getattr(cfg, "TORABI_MORPH_LS_TRANSIENT_PROB", 0.50)),
                    ls_transient_strength=tuple(getattr(cfg, "TORABI_MORPH_LS_TRANSIENT_STRENGTH", (0.04, 0.18))),
                    ls_transient_high_hz=float(getattr(cfg, "TORABI_MORPH_LS_TRANSIENT_HIGH_HZ", 1000.0)),
                    ls_texture_prob=float(getattr(cfg, "TORABI_MORPH_LS_TEXTURE_PROB", 0.35)),
                    ls_texture_snr_db=tuple(getattr(cfg, "TORABI_MORPH_LS_TEXTURE_SNR_DB", (30.0, 40.0))),
                    ls_texture_low_hz=float(getattr(cfg, "TORABI_MORPH_LS_TEXTURE_LOW_HZ", 150.0)),
                    ls_texture_high_hz=float(getattr(cfg, "TORABI_MORPH_LS_TEXTURE_HIGH_HZ", 1000.0)),
                    apply_hs_prob=float(getattr(cfg, "TORABI_MORPH_APPLY_HS_PROB", 0.30)),
                    hs_band_150_500_boost_db=tuple(getattr(cfg, "TORABI_MORPH_HS_BAND_150_500_BOOST_DB", (0.0, 1.5))),
                    hs_transient_prob=float(getattr(cfg, "TORABI_MORPH_HS_TRANSIENT_PROB", 0.15)),
                    hs_transient_strength=tuple(getattr(cfg, "TORABI_MORPH_HS_TRANSIENT_STRENGTH", (0.02, 0.08))),
                )

                synth_loader_dataset = Subset(synth_aug_dataset, synth_indices)

                print()
                print("=" * 76)
                print("TORABI-LIKE MORPHOLOGY AUGMENTATION V2 ENABLED ON REPLAY ONLY")
                print("=" * 76)
                print(f"prob                         : {getattr(cfg, 'TORABI_MORPH_PROB', 0.50)}")
                print(f"preserve source RMS           : {getattr(cfg, 'TORABI_MORPH_PRESERVE_SOURCE_RMS', True)}")
                print(
                    f"LS 150-1000 boost dB          : {getattr(cfg, 'TORABI_MORPH_LS_BAND_150_1000_BOOST_DB', (1.0, 4.0))}")
                print(
                    f"LS 500-1000 boost dB          : {getattr(cfg, 'TORABI_MORPH_LS_BAND_500_1000_BOOST_DB', (1.0, 5.0))}")
                print(
                    f"LS transient prob/strength    : {getattr(cfg, 'TORABI_MORPH_LS_TRANSIENT_PROB', 0.50)} / {getattr(cfg, 'TORABI_MORPH_LS_TRANSIENT_STRENGTH', (0.04, 0.18))}")
                print(
                    f"LS texture prob/SNR dB        : {getattr(cfg, 'TORABI_MORPH_LS_TEXTURE_PROB', 0.35)} / {getattr(cfg, 'TORABI_MORPH_LS_TEXTURE_SNR_DB', (30.0, 40.0))}")
                print(
                    f"LS texture band Hz            : {getattr(cfg, 'TORABI_MORPH_LS_TEXTURE_LOW_HZ', 150.0)} - {getattr(cfg, 'TORABI_MORPH_LS_TEXTURE_HIGH_HZ', 1000.0)}")
                print(f"HS apply prob                 : {getattr(cfg, 'TORABI_MORPH_APPLY_HS_PROB', 0.30)}")

            synth_loader = DataLoader(
                synth_loader_dataset,
                batch_size=args.batch_size,
                shuffle=True,
                num_workers=args.num_workers,
                pin_memory=True,
            ...
            if bool(getattr(cfg, "USE_SSL_PSEUDO", False)):
                if ssl_dataset is None:
                    raise RuntimeError("USE_SSL_PSEUDO=True but ssl_dataset was not initialized.")
                ssl_indices = list(range(len(ssl_dataset)))
                max_ssl_segments = getattr(cfg, "SSL_PSEUDO_MAX_SEGMENTS", None)
                if (
                    max_ssl_segments is not None
                    and int(max_ssl_segments) > 0
                    and int(max_ssl_segments) < len(ssl_indices)
                ):
                    rng = np.random.default_rng(args.seed + 1000 + fold_no)
                    selected_ssl = rng.choice(
                        len(ssl_indices),
                        size=int(max_ssl_segments),
                        replace=False,
                    )
                    ssl_indices = [ssl_indices[i] for i in sorted(selected_ssl)]
                ssl_loader = DataLoader(
                    Subset(ssl_dataset, ssl_indices),
                    batch_size=args.batch_size,
                    shuffle=True,
                    num_workers=args.num_workers,
                    pin_memory=True,
                ...
                print(
                    f"Fold {fold_no} SSL pseudo segments: "
                    f"{len(ssl_indices)} | batches={len(ssl_loader)}"
                )
        ...
        for epoch in range(args.finetune_epochs):
            ...
            if mixed_training:
                mixed_log = (
                    f" | pTarget={train_m.get('mixed_p_v2', float('nan')):.2f}"
                    f" | bTarget={int(train_m.get('mixed_v2_batches', 0))}"
                    f" | bSyn={int(train_m.get('mixed_synth_batches', 0))}"
                    f" | bSSL={int(train_m.get('mixed_ssl_batches', 0))}"
                    f" | pSSLrem={train_m.get('ssl_pseudo_prob_in_remainder', 0.0):.2f}"
                    f" | wSSL={train_m.get('ssl_pseudo_weight', 0.0):.3f}"
                ...
                torch.save(
                    {
                        ...
                        "config": {
                            ...
                            "SYNTH_SOURCE_DISJOINT_SPLIT_CSV": getattr(cfg, "SYNTH_SOURCE_DISJOINT_SPLIT_CSV", None),
                            "SYNTH_REPLAY_FOLD_NO": getattr(cfg, "SYNTH_REPLAY_FOLD_NO", None),
                            "SYNTH_REPLAY_SPLIT": getattr(cfg, "SYNTH_REPLAY_SPLIT", None),
                            "REQUIRE_SYNTH_TRAIN_SPLIT": getattr(cfg, "REQUIRE_SYNTH_TRAIN_SPLIT", None),
                            ...
                            "USE_SSL_PSEUDO": getattr(cfg, "USE_SSL_PSEUDO", False),
                            "SSL_PSEUDO_DIR": getattr(cfg, "SSL_PSEUDO_DIR", None),
                            "SSL_PSEUDO_WEIGHT": getattr(cfg, "SSL_PSEUDO_WEIGHT", None),
                            "SSL_PSEUDO_PROB_IN_REMAINDER": getattr(cfg, "SSL_PSEUDO_PROB_IN_REMAINDER", None),
                            "SSL_PSEUDO_MAX_SEGMENTS": getattr(cfg, "SSL_PSEUDO_MAX_SEGMENTS", None),
                            "USE_SOURCE_DISJOINT_SPLIT": use_source_disjoint,
                            "SOURCE_DISJOINT_SPLIT_CSV": getattr(cfg, "SOURCE_DISJOINT_SPLIT_CSV", None),
                            "N_FOLDS": cfg.N_FOLDS,
                            "SPLIT_SEED": cfg.SEED,
    ...
    summary_title = (
        "Source-disjoint mixed fine-tuning results"
        if use_source_disjoint
...
def main():
    ...
    print(f"TRAIN SCRIPT VERSION: {SSL_TRAIN_SCRIPT_VERSION}")
